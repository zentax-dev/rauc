...
gboolean load_config(const gchar *filename, RaucConfig **config, GError **error)
{
	...
	c->lock_good_slot = g_key_file_get_boolean(key_file, "system", "lock-good-slot", &ierror);
	if (g_error_matches(ierror, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND)) {
		c->lock_good_slot = FALSE;
		g_clear_error(&ierror);
	} else if (ierror) {
		g_propagate_error(error, ierror);
		return FALSE;
	}
	g_key_file_remove_key(key_file, "system", "lock-good-slot", NULL);
