...
``filename`` (required)
  ...
  .. important::
    RAUC uses the file name extension and the slot type to decide
    how to extract the image file content to the slot.
...
.. note::
  ...
  Use the explicit ``type`` field instead.
  For backward compatibility, RAUC can still derive the image type
  from file name extensions, but this could get removed in a future version

RAUC primarily uses the explicitly configured ``type`` field to determine how to
process images during installation.
